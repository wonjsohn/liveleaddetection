...
def get_arguments():
    ...
    help="marker: Specify the tracking algorithm. (e.g. el_object, cl_object) ")
    ...
    ap.add_argument("-vir", "--virtual", type=int,  default="0",
    help="Virtual display instead of webcam display .")
    ap.add_argument("-pth", "--path", type=str,  default="diag",
    help="Path of movement. Diagonal:diag, horizontal:hori, curve:cur")
    ap.add_argument("-pt", "--practice", type=int, default="0",
    help="Practice stage (in pygame)")
    ap.add_argument("-pb", "--practice_boardtask", type=int, default="0",
    help="Practice before the p2p or fig8 task.")
    ap.add_argument("-dc", "--discrete_control", type=int, default="0",
    help="Discrete control of pygame")
    ap.add_argument("-kc", "--keyboard_control", type=int, default="0",
    help="Keyboard control of pygame")
    ap.add_argument("-sid", "--subject", type=str, default="subj0",
    help="subject ID")
    ap.add_argument("-nt", "--note",  type=int, default="0",
    help="Leave note if necessary")
    ap.add_argument("-gnt", "--gamenote", type=int, default="0",
    help="Leave game note if necessary")
    ap.add_argument("-gt", "--gametest", type=int, default="0",
    help="Game Test mode: 1 if using mouse / keyboard")
    ap.add_argument("-br", "--bulkrecording", type=int, default="-1",
    help="If recording many trials at once (bulk recording)")
    ...
    if args["mode"] == "pp":  # PP: post processing
        ...
        args["thread"] = 0   #  must
        args["trace"] = 1           # option
        args["linetrace"] = 0      # option
        args["marker"] = "multi_small_blob_object"  # option
        args["tasktype"] = "p2p"  # option
    elif args["mode"] == "play":  #
        #del args["mode"] #remove it
        args["note"] =1 # leave note on.
        args["clock"] = 1
        ...
        args["trace"] = 1
        args["linetrace"] = 0
        args["marker"] = "multi_small_blob_object"  # option "camshift_multi_object", "multi_small_blob_object, ""small_blob_object+kalman", "small_blob_object", "cl_object", "el_object", "cs_object", "cl_object+kalman", "el_object_dual", "el_object+kalman"
