...
def detectShapesInTable(img_name, centerx, centery, table_halfw, table_halfh ):
        path_filename = os.path.join("Output", "snapshots", img_name) # FIX:CROSS PLATFORM
        ...
        # find contours in the edged image, keep only the largest
        # ones, and initialize our screen contour
        # im2, contours, hierarchy = cv2.findContours(edged.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # (obsolete)
...
""" For Live Lead Migration Detecion (no need to detect circle etc)"""
def justReturnSelectedImage(img_name, centerx, centery, table_halfw, table_halfh):
    path_filename = os.path.join("Output", "snapshots", img_name)  # FIX:CROSS PLATFORM
    print("path ", path_filename)
    img2 = cv2.imread(path_filename)
    print(img_name)

    shot_frame = img2.copy()
